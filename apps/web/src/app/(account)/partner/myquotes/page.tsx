...
export default function QuotesPage() {
  ...
  return (
    <div style={{ paddingBottom: '32px' }}>
      ...
      {/* View Switcher */}
      <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', backgroundColor: '#f1f5f9', padding: '6px', borderRadius: '10px', border: '1px solid #e2e8f0', width: 'fit-content', flexWrap: 'wrap' }}>
        <button
        ...
        {/* Calendar Toggle - Only for Caterer */}
        {currentView === 'caterer' && (
          <button
            onClick={() => setCalendarView(!calendarView)}
            style={{
              padding: '8px 12px',
              backgroundColor: calendarView ? '#667eea' : '#f1f5f9',
              color: calendarView ? 'white' : '#667eea',
              border: '1px solid #e2e8f0',
              borderRadius: '8px',
              fontWeight: '600',
              cursor: 'pointer',
      ...
      {currentView === 'customer' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {filteredQuotes.length === 0 ? (
            <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '40px 20px', textAlign: 'center', border: '1px solid #e2e8f0' }}>
              <p style={{ fontSize: '16px', color: '#64748b', margin: 0 }}>No quotes yet</p>
            </div>
          ) : (
            filteredQuotes.map(quote => {
              const config = statusConfig[quote.status];
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '16px' }}>
                    <div>
                      <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#1e293b', margin: 0, marginBottom: '8px' }}>
                        {quote.mealType}
                      </h3>
                  ...
                  <div style={{ backgroundColor: '#f0f9ff', borderRadius: '8px', padding: '12px', marginBottom: '16px', border: '1px solid #bfdbfe' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: '12px', color: '#64748b' }}>Quoted Price</span>
                      <span style={{ fontSize: '16px', fontWeight: '900', color: '#2563eb' }}>₹{quote.quotedPrice}</span>
                    </div>
                  </div>

                  {/* Timeline */}
                  <div style={{ fontSize: '12px', color: '#64748b' }}>
                    <p style={{ margin: '0 0 8px 0' }}>📅 Requested: {quote.createdAt}</p>
                    <p style={{ margin: '0 0 8px 0' }}>⏰ Expires: {quote.expiresAt}</p>
                    {quote.approvedAt && <p style={{ margin: 0 }}>✓ Approved: {quote.approvedAt}</p>}
                    {quote.startDate && <p style={{ margin: 0 }}>📌 Starts: {quote.startDate}</p>}
                  </div>
                </div>
              );
            })
