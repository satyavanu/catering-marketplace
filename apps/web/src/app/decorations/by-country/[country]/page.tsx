...
import { useDecorationsByCountry } from '@catering-marketplace/query-client';
import { useParams } from 'next/navigation';
import { StarIcon, HeartIcon } from '@heroicons/react/24/outline';
import Link from 'next/link';

export default function DecorationsByCountryPage() {
  const params = useParams();
  const country = decodeURIComponent(params.country as string);
  const [pagination, setPagination] = useState({ page: 1, limit: 12 });
  const { data: decorations = [], isLoading, error } = useDecorationsByCountry(country, pagination);
  const [favorites, setFavorites] = useState<string[]>([]);

  if (isLoading) return <div style={{ padding: '2rem', textAlign: 'center' }}>Loading decorations in {country}...</div>;
    ...
    <div style={{ backgroundColor: '#f8fafc', minHeight: '100vh' }}>
      ...
      >
        <h1 style={{ fontSize: '42px', fontWeight: '800', margin: 0 }}>
          Decorations in {country}
        </h1>
        <p style={{ fontSize: '16px', opacity: 0.9, margin: '12px 0 0 0' }}>
          Find the best decoration services in {country}
        </p>
      </div>
